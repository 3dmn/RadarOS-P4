...
| Zasięg radaru | `select` | 25 / 50 / 100 / 200 / 400 km |
...
Każda encja `switch`/`select`/`number` używa tego samego schematu topików: `homeassistant/<component>/<node_id>/<object_id>/set` (komenda, publikowana przez Ciebie/HA) oraz `.../state` (stan, retained, publikowany przez urządzenie). Dla **Priorytetu wojskowego**: `homeassistant/switch/radaros_p4/mil_priority/set` (payload `ON`/`OFF`) oraz `homeassistant/switch/radaros_p4/mil_priority/state` (`<node_id>` domyślnie `radaros_p4`, konfigurowalny w zakładce **MQTT**).
...
| Powód ostatniego resetu | Sensor diagnostyczny - dlaczego urządzenie ostatnio się zrestartowało (Power On, Software Reset, Crash/Panic, Watchdog, Brownout, Deep Sleep itd.) |
